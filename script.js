...
function affichagedonnees(data) {
  ...
  data.forEach((pays) => {
    ...
    if (pays.currencies) {
      ...
      monnaie.textContent = "Monnaie non disponible";
    }

    if (pays.languages) {
      let languesListe = Object.values(pays.languages).join(", ");
      langue.textContent = `Langue(s) : ${languesListe}`;
    } else {
      langue.textContent = "Langues non disponibles";
    }
